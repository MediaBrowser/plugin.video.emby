...
def set_ListItem_from_Kodi_database(KodiItem, Path=None):
    ...
    if KodiItem['mediatype'] in ("episode", "movie", "musicvideo", "tvshow", "season", "set"):
        set_ListItem_MetaData('video', ListItem, KodiItem)
    else:
        set_ListItem_MetaData('music', ListItem, KodiItem)
...
def set_ListItem(item, ServerId, Path, get_shortdate):
    ...
    if item['Type'] == 'Folder' or item.get('NodesMenu', False):
        ...
        set_ListItem_MetaData('video', listitem, metadata)
        ...
        Properties = {'embyserverid': str(ServerId), 'embyid': str(item['Id']), 'IsFolder': 'false', 'IsPlayable': 'true', 'TotalTime': str(item['RunTimeTicks']), 'ResumeTime': item['UserData']['PlaybackPositionTicks']}
        set_ListItem_MetaData('video', listitem, metadata)
        ...
        set_ListItem_MetaData('music', listitem, metadata)
        ...
        Properties = {'embyserverid': str(ServerId), 'embyid': str(item['Id']), 'IsFolder': 'true', 'IsPlayable': 'true', 'TotalTime': str(item['RunTimeTicks'])}
        set_ListItem_MetaData('music', listitem, metadata)
        ...
        metadata = {'title': item['Name'], 'picturepath': item['KodiArtwork']['poster'], 'exif:width': item['Width'], 'exif:height': item['Height'], 'date': get_shortdate(item['PremiereDate']), 'count': item['UserData']['PlayCount'], 'resolution': f"{item['Width']}, {item['Height']}"}
        Properties = {'embyserverid': str(ServerId), 'embyid': str(item['Id']), 'IsFolder': 'false', 'IsPlayable': 'true'}
        set_ListItem_MetaData('pictures', listitem, metadata)
    ...
    if 'Streams' in item:
        ...
        for track in item['Streams'][0]['Subtitle']:
            set_ListItem_StreamInfo('subtitle', listitem, {'language': track['language']})
