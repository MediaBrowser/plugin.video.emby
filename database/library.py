...
class Library:
    ...
    def worker_update(self):
        ...
        with DBLock:
            ...
            embydb = db_open.DBOpen(Utils.DatabaseFiles, self.EmbyServer.server_id)
            ItemCounter = embydb.get_UpdateItem_number_of_records()
            Counter = 0

            while True:
                UpdateItems = embydb.get_UpdateItem(Utils.limitIndex)  # 500 item packages by SQL query limitation
    ...
    def fast_sync(self, plugin):
        ...
        for LibraryId, library_type, library_name in self.Whitelist:
            ...
            if 'Items' in Items:
                ItemCounter = 0
                ItemTemp = len(Items['Items']) * [None]  # allocate memory for array (much faster than append each item)
