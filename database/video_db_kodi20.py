...
def add_streams(Cursor, file_id, videostream, audiostream, subtitlestream, runtime):
    for track in videostream:
        Cursor.execute("INSERT OR REPLACE INTO streamdetails(idFile, iStreamType, strVideoCodec, fVideoAspect, iVideoWidth, iVideoHeight, iVideoDuration, strStereoMode, strVideoLanguage, strHdrType) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)", (file_id, 0, track['codec'], track['aspect'], track['width'], track['height'], int(runtime), track['3d'], track['language'], track['hdrtype']))
